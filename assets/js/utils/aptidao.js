jQuery(document).ready(function () {
    ...
    jQuery('#aptidao').submit(function () {
        var now = new Date(Date.now());
        var mes =  now.getMonth() + 1;
        var formatted = now.getDate() + "/" + mes + "/" +now.getFullYear()+ "-" + now.getHours() + ":" + now.getMinutes() + ":" + now.getSeconds();
        jQuery("#tempo").val(formatted);
        var $formArray = jQuery('#aptidao').serializeArray();
        jQuery.each($formArray, function (index, field) {
            $data[field.name] = field.value;
        });
        db.collection('grupos').doc(str).collection('alunos').doc(str2).collection('testes').doc('1').collection('Paq-C e Paq-A').doc().set($data).then(function () {
